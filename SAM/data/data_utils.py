...
def get_img_transform(train=False, image_size=224, crop=False, jitter=0, use_flip=True):
...
def get_pre_transform(image_size=224, crop=False, jitter=0, use_flip=True):
    if crop:
        img_transform = [transforms.RandomResizedCrop(image_size, scale=[0.8, 1.0])]
    else:
        img_transform = [transforms.Resize((image_size, image_size))]
...
def colorful_spectrum_mix(img1, img2, label1, label2, domain1, domain2, alpha, ratio=1.0):
    """Input image size: ndarray of [H, W, C]"""
    lam = np.random.uniform(0, alpha)

    if (label1 == label2).all() and domain1 == domain2:
        f_abs = False
        f_pha = True
    if (label1 != label2).any() and domain1 == domain2:
        f_abs = False
        f_pha = True
    if (label1 == label2).all() and domain1 != domain2:
        f_abs = True
    ...
    if f_pha:
        ...
        img2_pha[h_start:h_start + h_crop, w_start:w_start + w_crop] = \
            lam * img1_pha_[h_start:h_start + h_crop, w_start:w_start + w_crop] + (1 - lam) * img2_pha_[
                                                                                            h_start:h_start + h_crop,
                                                                                            w_start:w_start + w_crop]
        label21 = lam * label2 + (1 - lam) * label1
        label12 = lam * label1 + (1 - lam) * label2
    else:
        label21 = label1
        label12 = label2
    ...
    return img21, img12, label21, label12