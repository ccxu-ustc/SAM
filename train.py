...
class Trainer:
    ...
    def _do_epoch(self):
        ...
        for it, (batch, label, domain) in enumerate(self.train_loader):
            # preprocessing
            batch = torch.cat(batch, dim=0).to(self.device)  # [64, 3, 224, 224]
            label = torch.cat(label, dim=0).to(self.device)  # [64]
            ...
            features = self.encoder(batch) # [64, 2048]
            scores = self.classifier(features) # [64, 7]
            with torch.no_grad():
